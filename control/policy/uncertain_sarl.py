...
class UNCERTAIN_SARL(Policy):
    def __init__(self):
        super().__init__()
        self.name = 'UNCERTAIN_SARL'
        self.trainable = True
    ...
    def configure(self, config):
        ...
        self.adjust_dist = config.getboolean('reward', 'adjust_dist')
        self.dist_slope = config.getfloat('reward', 'dist_slope')
        self.dist_intercept = config.getfloat('reward', 'dist_intercept')
        ...
        logging.info('Policy: Uncertainty-Aware SARL')
    ...
    def compute_reward(self, nav, obs, eps=None):
        if self.adjust_dist and eps is not None:
            # check if reaching the goal
            if la.norm((nav.px - nav.gx, nav.py - nav.gy)) < nav.radius:
                return 1 # reaching goal reward

            # comupte discomfort penalty and check for collision
            reward = 0
            collision = False
            for i, ob in enumerate(obs):
                dist = la.norm((nav.px - ob.px, nav.py - ob.py)) - nav.radius - ob.radius
                disc_dist = self.dist_intercept + self.dist_slope * eps[i]
                if dist < 0:
                    return -0.25 # collision penalty
                elif dist < disc_dist:
                    reward += (dist - disc_dist) * 0.5 * self.time_step
            return reward # 0 or discomfort penalty
        else:
            # check for collision
            dmin = float('inf')
            collision = False
    ...
    def transform(self, state, eps):
        state_tensor = torch.cat([torch.Tensor([state.self_state + human_state]).to(self.device)
                                  for human_state in state.human_states], dim=0)
        state_tensor = self.rotate(state_tensor, torch.from_numpy(eps))
        return state_tensor

    def rotate(self, state, eps):
        ...
        eps = torch.reshape(eps, dg.size())
        new_state = torch.cat([dg, v_pref, theta, radius, vx, vy, px1, py1, vx1, vy1, radius1, da, radius_sum, eps], dim=1)
    ...
    def predict(self, state, eps):
        ...
        if state.human_states:
            ...
            if self.phase == 'train' and probability < self.epsilon:
                ...
                for free, action in zip(self.free_directions, self.action_space):
                    ...
                    if self.query_env:
                        ...
                        reward = self.compute_reward(next_self_state, next_human_states, eps)
                    batch_next_states = torch.cat([torch.Tensor([next_self_state + next_human_state]).to(self.device)
                                                  for next_human_state in next_human_states], dim=0)
                    rotated_batch_input = self.rotate(batch_next_states, torch.from_numpy(eps)).unsqueeze(0)
            ...
            if self.phase == 'train':
                self.last_state = self.transform(state, eps)
